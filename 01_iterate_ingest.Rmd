...
In this lesson we'll learn how to ingest multiple CSV files into a single data frame, also known as a _tibble_.  To do this, I'll use a file system package, `fs` that helps manipulate the files.  I'll introduce the technique of mapping over rows using the `purrr` package, but the purr lesson come later.  
